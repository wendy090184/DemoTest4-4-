...
public class ExeclUtils {
    ...
    private static final String EXCEL_XLSX = "xlsx";//2007以上
    private static SimpleDateFormat fmt = new SimpleDateFormat("yyyy-MM-dd");//定义日期格式
    ...
    public static List<TestCase> getUITestData(String fileName){
    	...
    	try{
	        ...
	        for(Row row:sheet){
	        	...
	        	if(row.getCell(0).toString().equals("") || row.getCell(1).toString().equals("notRun")){  //如果第一个单元格是空的，或者是notRun，就不执行
	        		continue;  
                }
